...
tf.app.flags.DEFINE_integer('max_epoch',500,'maximum of training epochs')
...
def main(_):
	...
	def train_kg(coord):
		...
		batch_size = (int(FLAGS.ent_total) // int(FLAGS.nbatch_kg))
...
#							s1+= 1.0
				...
				print ("batch %d step %d time %s | loss : %f, accuracy: %f" % (one_epoch, i, time_str, loss, s2 / tot2))
			...
			if (one_epoch + 1) % save_epoch == 0:
				...
				path = saver.save(sess,FLAGS.model_dir+FLAGS.model+str(FLAGS.katt_flag)+transX)
