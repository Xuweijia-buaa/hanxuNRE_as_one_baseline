...
embedding_size=config['word_size'] if use_embedding else 300
...
contain_dev_file='/media/xuweijia/06CE5DF1CE5DD98F/useful_old_data/new_NER_data/e_30/up_30/dev_contain_e.json'
contain_test_file='/media/xuweijia/06CE5DF1CE5DD98F/useful_old_data/new_NER_data/e_30/up_30/test_contain_e.json'
if dev:
...
FLAGS.katt_flag=int(FLAGS.katt_flag)
print( FLAGS.model_dir+FLAGS.model+str(FLAGS.katt_flag)+transX)
saver.restore(sess, FLAGS.model_dir+FLAGS.model+str(FLAGS.katt_flag)+transX)
...
for i,sample in enumerate(samples):
    ...
    if eid2idx.get(e1_id)!=None and pid2idx.get(p_id)!=None:
        ...
        prediction=predictions[1] if predictions==e1_id else predictions[0] 
        correct=prediction in ans_id
        exclude_e1+=correct
...
print('transX:{} mode:{} size:{} e{}:'.format(transX,modeX,embedding_size,ee))
