...
class Config(object):
	...
	def init(self):
		...
		if self.test_flag:
			...
			self.correct = np.zeros(1,dtype = np.int64) #placeholder 1 pos
	...
	def set_export_steps(self, steps):#To export results every few rounds
	...
	def save_pytorch(self): # save state_dict ,is a dict,but could load directly
		torch.save(self.trainModel.state_dict(), self.exportName)

	def restore_pytorch(self):# just load state_dict from pt
	...
	def get_parameter_lists(self):
		return self.trainModel.cpu().state_dict() # original parameter dict,astore two embeddings
	...
	def get_parameters(self, mode = "numpy"):
		res = {}
		lists = self.get_parameter_lists() # dict: 'ent_embeddings.weight','rel_embeddings.weight'
        # new dict res,store two embeddings
		for var_name in lists:
			if mode == "numpy":
				res[var_name] = lists[var_name].numpy() # save as numpy seperately
			if mode == "list":
				res[var_name] = lists[var_name].numpy().tolist() # save as numpy seperately
			else:
				res[var_name] = lists[var_name]   # tensor float
		return res
	...
	def save_parameters(self, path = None):
		if path == None:
			path = self.out_path
		f = open(path, "w")
		f.write(json.dumps(self.get_parameters("list")))  # write list/numpy dict into json file (out_path)
		f.close()

	def set_parameters_by_name(self, var_name, tensor): # defalut is list/numpy/tensor  turn to numpy.then load para for test Model
		self.trainModel.state_dict().get(var_name).copy_(torch.from_numpy(np.array(tensor)))

	def set_parameters(self, lists): # set_parameters .lists is a dict that stored in json file, lists/numpy
		for i in lists:
	...
	def run(self):
		...
		for epoch in range(self.train_times):
			res = 0.0
			for batch in range(self.nbatches):# 每个ｅｐｏｃｈ整体采多少次样（训练多少次）
				self.sampling()
		...
		if self.exportName != None:
			self.save_pytorch()                 # # save state_dict to pt,is a dict,but could load directly
		if self.out_path != None:
			self.save_parameters(self.out_path) # only save paras dict to json

	def test(self):
		if self.importName != None:
			self.restore_pytorch() # just load trained state_dict from pt
		# total_test = self.lib.getTestTotal()　　　　
		# total_test=100　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　
		for epoch in range(total_test):
			self.lib.getTailBatch(self.test_h_addr, self.test_t_addr, self.test_r_addr)      
			res = self.trainModel.predict(self.test_h, self.test_t, self.test_r)
			self.lib.testTail(res.data.numpy().__array_interface__['data'][0],self.correct)
			match+=self.correct
		total+=1   
#			if self.log_on:
#				print epoch
