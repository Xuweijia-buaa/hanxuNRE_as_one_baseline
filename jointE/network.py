...
#			use_embedding=False
		...
		self.label_index = tf.placeholder(dtype=tf.int32,shape=[None], name='label_index')                            # real relation idx for each sample/ B
		...
		self.label = tf.placeholder(dtype=tf.float32,shape=[FLAGS.batch_size, self.num_classes], name='input_label')  # B,h, real pos value 1, others value 0
...
#            
		with tf.name_scope("embedding-layers"):
			# word embeddings
			#temp_word_embedding = tf.get_variable(initializer=word_embeddings[FLAGS.ent_total:,:],name = 'temp_word_embedding') if use_embedding else \
...
class CNN(NN):
	...
	def __init__(self, use_embedding,embedding_size,is_training,word_embeddings, simple_position = False):
		...
		with tf.name_scope("conv-maxpool"):
			...
			x = tf.layers.conv2d(inputs = input_sentence, filters=FLAGS.hidden_size, kernel_size=[1,int(FLAGS.win_size)], strides=[1, 1], padding='same', kernel_initializer=tf.contrib.layers.xavier_initializer_conv2d()) 
		...
		with tf.name_scope("loss"):
			...
			self.correct_predictions = tf.equal(self.predictions, tf.argmax(self.label, 1))  # B, if each sample predict true(1), else 0
