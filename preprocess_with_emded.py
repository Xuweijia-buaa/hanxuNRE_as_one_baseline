...
#embedding_file='/media/xuweijia/06CE5DF1CE5DD98F/word_embedding/word2vec_glove.txt'               # 400000,100
...
def build_dict(samples,embedding_file):
    ...
    # make word_vec, add to e_dict
    print ('reading word embedding data...')
    f = open(embedding_file, "r")
    total, size = f.readline().strip().split()[:2]
    total = (int)(total)                           # all words in embed_file
    ...
    vec = np.ones((total + len(e_dict),embedding_size), dtype = np.float32)
    for i in range(total):
        content = f.readline().strip().split()
        word_dict[content[0].lower()] = len(word_dict)
        vec[i + len(e_dict),:]=np.array(list(map(float, content[1:])), dtype='float32')
#		for j in range(word_size):
#			vec[i + len(e_dict)][j] = (float)(content[j+1])
    f.close()
...
word_dict,e_dict,r_dict,E,V,I,T,triple_set=build_dict(samples,embedding_file)
