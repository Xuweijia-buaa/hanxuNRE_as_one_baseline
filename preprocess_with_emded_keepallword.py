...
#embedding_file='/media/xuweijia/06CE5DF1CE5DD98F/word_embedding/word2vec_glove.txt'                # 400000,100 n_in_V:71647/149532    68124/149812
#embedding_file='/media/xuweijia/06CE5DF1CE5DD98F/word_embedding/hanxu_vec.txt'                    # 114042 50  n_in_V:40194/149532      38659/149812
embedding_file='/media/xuweijia/06CE5DF1CE5DD98F/word_embedding/glove840B300d.txt'                # glove 300  n_in_V: 77076/149532      73563/149812
name=embedding_file.split('/')[-1].split('.')[:-1][0]+'_keepwd'

transX_dir='transX_embedding_{}'.format(name)
...
import copy
def build_dict(samples,embedding_file):
    ...
    #vec = np.random.random((len(e_dict)+V+2,embedding_size), dtype = np.float32)
    if name=='word2vec_glove_keepwd' or name=='hanxu_vec_keepwd':
        f = open(embedding_file, "r")
        total, size = f.readline().strip().split()[:2]
        embedding_size = (int)(size)
        vec = np.random.randn(len(word_dict),embedding_size)  # N(0,1)
        total=int(total)
        for i in range(total):
            ...
            if w in word_dict:
                n_in_E+=1
                vec[word_dict[w],:]=np.array(list(map(float, content[1:])), dtype='float32')
    elif name=='glove840B300d_keepwd':
        embedding_size=300
        vec = np.random.randn(len(word_dict),embedding_size)
        vec_counts=dict()
        with open(embedding_file) as f:
            for line in f:
                ...
                w = parsed[0].lower()
                if w in word_dict:
                    if w not in already_set:
                        vec[word_dict[w],:]=np.array([float(i) for i in parsed[1:]],dtype='float32')
                        already_set.add(w)
                        vec_counts[w] = 1
                        n_in_E+=1
                    else:
        ...
        for w, c in vec_counts.items():
             vec[word_dict[w]]=vec[word_dict[w]]/c
    print('n_in E {}/{}'.format(n_in_E,V+2))       
    np.save(transX_dir+'/vec.npy', vec)
    ...
    f.write(json.dumps({"word2id":word_dict,"relation2id":r_dict,"e_dict":e_dict,"word_size": embedding_size, "fixlen":fixlen, "maxlen":maxlen, "entity_total":E, "word_total":I,"T":T, "rel_total":R, "textual_rel_total":R}))
    f.close()
    ...
    return word_dict,e_dict,r_dict,E,V,I,T,triple_set,n_in_E


word_dict,e_dict,r_dict,E,V,I,T,triple_set,n_in_E=build_dict(samples,embedding_file)
id2w=dict(zip(word_dict.values(),word_dict.keys()))
print('n_inE {}/{}'.format(n_in_E,V+2)) 
